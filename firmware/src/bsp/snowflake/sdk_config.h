...
#define CLOCK_ENABLED 0  // todo zmieniłem z 1
#endif
#if CLOCK_ENABLED
...
// <i> Optimization flag. Registers used by TWIS are shared by other peripherals. Normally, during initialization driver tries to clear all registers
// to known state before doing the initialization itself. This gives initialization safe procedure, no matter when it would be called. If you activate
// TWIS only once and do never uninitialize it - set this flag to 1 what gives more optimal code.
...
// <i> Synchronous mode would be used in specific situations. And it uses some additional code and data memory to safely process state machine by
// polling it in status functions. If this functionality is not required it may be disabled to free some resources.
...
// <i> Time of single measurement can be calculated as T = (1/2)*MEASUREMENT_PERIOD*(1/f_OSC) where f_OSC = I_SOURCE / (2C*(VUP-VDOWN) ). I_SOURCE,
// VUP and VDOWN are values used to initialize COMP and C is capacitance of used pad.
